...
void ctsu_self_set(void)
{
	...
	ctsu_self_reg_set_multi();	//  CTSUレジスタ設定 ( 4ch計測 TS09,TS10,TS11,TS13)
...
void ctsu_self_reg_set_multi(void)
{
	...
	CTSUCHAC1_bit.no1 = 1;   // TS09 計測対象
	CTSUCHAC1_bit.no2 = 1;   // TS10 計測対象
	CTSUCHAC1_bit.no3 = 1;   // TS11 計測対象
...
//
// 1) P22 = TS09
//   PM (ポート・モード・レジスタ) : PM2 bit2= 1 ( 入力モード（出力バッファ・オフ）) 
//   P (ポート・レジスタ) : P2 bit2 = 0 (ロー・レベルを入力)
//   TSSEL(タッチ端子機能選択レジスタ) : TSSEL1 bit1 = 1 (タッチ端子機能として使用)
//
// 2) P21 = TS10
...
//
// 3) P20 = TS11
//   PM (ポート・モード・レジスタ) : PM2 bit0= 1 ( 入力モード（出力バッファ・オフ）) 
//   P (ポート・レジスタ) : P2 bit0 = 0 (ロー・レベルを入力)
//   TSSEL(タッチ端子機能選択レジスタ) : TSSEL1 bit3 = 1 (タッチ端子機能として使用)
//
// 4) P41 = TS13
...
void touch_port_set(void)
{
				...
				// P22 / TS09
	PM2_bit.no2 = 1;	// P22は、入力モード（出力バッファ・オフ）
	P2_bit.no2 = 0;		// ロー・レベルを入力
	TSSEL1_bit.no1 = 1;	// タッチ端子機能として使用
	...
	TSSEL1_bit.no2 = 1;	// タッチ端子機能として使用
				...
				// P20 / TS11
	PM2_bit.no0 = 1;	// P20は、入力モード（出力バッファ・オフ）
	P2_bit.no0 = 0;		// ロー・レベルを入力
	TSSEL1_bit.no3 = 1;	// タッチ端子機能として使用
	
				// P41 / TS13
	PM4_bit.no1 = 1;	// P20は、入力モード（出力バッファ・オフ）
